...
class GoogleDriveAuthLocal extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'google-drive:auth-local';
     ...
     * @var string
     */
    protected $description = 'Autentica com o Google Drive usando OAuth 2.0 para desenvolvimento local';
    ...
    {
        $this->info('Iniciando autenticação com Google Drive (modo local)...');

        try {
            $client = new Client();
            $credentialsPath = storage_path('app/google-credentials-localhost.json');
            
            if (!file_exists($credentialsPath)) {
                $this->error("Arquivo de credenciais localhost não encontrado em: {$credentialsPath}");
                $this->info("Usando credenciais padrão...");
            ...
            $this->line($authUrl);
            $this->newLine();
            $this->info('Após autorizar, você verá uma página com um código.');
            $this->info('Copie o código e cole aqui.');
            $this->newLine();
            ...
            if (array_key_exists('error', $accessToken)) {
                ...
                if (isset($accessToken['error_description'])) {
                    $this->error('Descrição: ' . $accessToken['error_description']);
                }
                return 1;
            }
